...
\section{额外配置}
本文档中大量的插图使用\lstinline{robust-externalize}宏包独立编译。为了减少GitHub Actions的编译时间，使用了多线程选项\lstinline{compile in parallel}。该选项在Windows系统不可用，因此如果你使用Windows系统，需要注释掉该行。该行位于\lstinline{main.tex}的\lstinline|\robExtConfigure|参数中。
...
\begin{itemize}
  ...
  \item \lstinline|\placeXorGate[灯数]{灯的状态}{门x}{门y}|：绘制一个异或门。
  ...
  \item \lstinline|\placePressurePlate{类型}{x}{y}|：绘制一个普通压力板，因为压力板的贴图有一个偏移。例如红压力板：\lstinline|\placePressurePlate{Red}{0}{0}|。
  \item \lstinline|\placeWeightedPressurePlate{类型}{状态}{x}{y}|：绘制一个测重压力板，因为其贴图有偏移。例如弹起的粉色测重压力板：\lstinline|\placeWeightedPressurePlate{Pink}{Off}{0}{0}|。
  \item \lstinline|\placeTealPressurePad{方向}{x}{y}|：绘制一个青绿压力垫板，因为其贴图有偏移。方向表示该垫板紧贴的实体块方向，取值为\lstinline|{T,B,L,R}|。
...
\begin{itemize}
  ...
  \item \lstinline{water}：画一片水。例如\lstinline{\draw[water] (0,0) rectangle (3,2);}。这个模板实际上就是蓝色滤镜。
  \item \lstinline{actuated}：被虚化。和\lstinline{water}类似，这实际上就是黑色滤镜。
\end{itemize}
