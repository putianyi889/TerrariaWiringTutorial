...
\begin{itemize}
  \item \lstinline|\placetile{图格名称}{x}{y}|：以\lstinline{(x,y)}为中心绘制一个大小为$1\times 1$的图格。图格名称即为\path{textures/tile/}目录下的文件名（不包含后缀）。例如\lstinline|\placetile{Torch_On}{1}{1}|。
  \item \lstinline|\placetiles{图格名称}{x1/y1,x2/y2,...}|：在很多位置绘制同一个图格。例如\lstinline|\placetiles{Torch_Off}{1/1,2/2,3/3}|。
  \item \lstinline|\placetileGroupN{图格名称}{x}{y}|：绘制尺寸大于$1\times 1$的图格。\lstinline{N}为宽度，\lstinline{(x,y)}为左下角图格的中心。例如\lstinline|\placetileGroup2{Chest}{1}{1}|。
  \item \lstinline|\placeAndGate[灯数]{灯的状态}{门x}{门y}|：绘制一个与门。灯数是可选参数，若不提供则由灯的状态推断。灯的状态是01串，从上到下。例如\lstinline|\placeAndGate{110}{1}{2}|。
  \item \lstinline|\placeSevenSegment{七段线状态}|：以\lstinline{(1,1)}为左下角图格中心绘制一个火把七段线。每根线长度为2。七段线状态是01串，顺序是顶-左上-右上-中-左下-右下-底。例如显示4：\lstinline|\placeSevenSegment{0111010}|。
\end{itemize}

\subsection{示例}

\begin{lstlisting}[style=latex]
\begin{tikzpicture}[scale=0.5, transform shape, x=16pt, y=16pt]
  \placetile{Torch_On}{1}{1}
  \placetile{Torch_Off}{3}{2}
  \draw[red] (1,1) -- (3,2);
\end{tikzpicture}%
\smarthfill{16}[4]%
\begin{tikzpicture}[scale=0.5, transform shape, x=16pt, y=16pt]
  \placetiles{Switch_Flat_Off}{1/1,2/1,2/2}
  \draw[blue] (0.5,0.5) rectangle (2.5,2.5);
\end{tikzpicture}%
\smarthfill{16}[4]%
\begin{tikzpicture}[scale=0.5, transform shape, x=16pt, y=16pt]
  \placetileGroup2{Gold_Chest}{1}{1}
  \placetileGroup3{Extractinator}{3}{2}
  \draw[xshift=12pt, yshift=12pt] (0,0) grid[step=1] (5,4);
\end{tikzpicture}%
\smarthfill{16}[4]%
\begin{tikzpicture}[scale=0.5, transform shape, x=16pt, y=16pt]
  \placeAndGate{1101}{1}{1}
  \placeAndGate{1111}{2}{1}
