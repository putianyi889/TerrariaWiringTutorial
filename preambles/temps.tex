...
\newcount\bitindex
\newif\ifallones
\newcommandAutoForward{\placeAndGate}[3]{%
	% 前两个参数是门的坐标，第三个参数是灯的状态，从下到上
	\pgfparserdef{and gate}{initial}0{
		\advance\bitindex by 1\relax
		\allonesfalse
		\placetile{Logic_Lamp_Off}{#1}{\bitindex}
	}
	\pgfparserdef{and gate}{initial}1{
		\advance\bitindex by 1\relax
		\pgfmathsetmacro\y{\bitindex+#2}
		\placetile{Logic_Lamp_On}{#1}{\bitindex}
	}
	\pgfparserdef{and gate}{initial};{\pgfparserswitch{final}}
	\bitindex=#2
	\allonestrue
	\pgfparserparse{and gate}#3;
	\ifallones
		\placetile{And_Gate_On}{#1}{#2}
	\else
		\placetile{And_Gate_Off}{#1}{#2}
	\fi
}
